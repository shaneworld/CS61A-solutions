    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'and': # and expressions
            ...
            if operator not in OPERATORS:
                while isinstance(operands.rest, Pair):
...
def floor_div(args):
    ...
    if isinstance(args, Pair):
        if not isinstance(args.rest, Pair):
            ...
            rest_op = args.rest.first
            if isinstance(rest_op, Pair):
                rest_op = calc_apply(OPERATORS[rest_op.first], rest_op.rest)
            div = args.first // rest_op
            result = Pair(div, args.rest.rest)
            return floor_div(result)
...
def eval_and(expressions):
    ...
    if isinstance(expressions, Pair):
        ...
        if isinstance(first, Pair):
            first = calc_apply(OPERATORS[first.first], first.rest)
        if first is False:
            return False
        ...
        if not rest:
            ...
            if isinstance(rest.first, Pair):
                return calc_apply(OPERATORS[rest.first.first], rest.first.rest)
            else:
                return rest.first
    else:
        return True
...
def eval_define(expressions):
    ...
    """
    if isinstance(expressions, Pair):
        fi = expressions.first
        re = expressions.rest
        if re.first in bindings:
            value = bindings[re.first]
            bindings[fi] = value
        else:
            bindings[fi] = re.first
        return fi
