...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    filter_list = [n for n in paragraphs if select(n)]
    try:
        return filter_list[k]
    except IndexError:
        return ''
    # END PROBLEM 1
...
def about(subject):
    ...
    def check_if_contain(paragraph):
        para_list = list(x.lower() for x in split(paragraph))
        for i in range(0, len(para_list)):
            for j in string.punctuation:
                para_list[i] = para_list[i].replace(j , '')
        for n in subject:
            if n in para_list:
                return True
        return False
...
def accuracy(typed, source):
    ...
    if length1 == 0 and length2 == 0:
        ...
        return 0.0
    # Count less one in case that out of range
    less = length1 if length1 < length2 else length2
    # Compare and count
    for i in range(0,less):
        if typed_words[i] == re.sub(r'[\n\t\a\f\r\v]', '', source_words[i]):
            correct_total += 1
    return round(correct_total / length1 * 100, 2)
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    # count the length of the typed word
    length = len(typed)
    standard = length / 5
    return round(standard * 60 / elapsed, 2)
    # END PROBLEM 4
...
def minimum_mewtations(typed, source, limit):
    ...
    # consider three cases
        ...
        substitude = minimum_mewtations(typed[1:], source[1:], limit - 1)
        return 1 + min(delete, add, substitude)
...
def fastest_words(match):
    ...
    # Initialize the result list and append list
    fastest = []
