...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current_place.is_hive:
            if self.lower_bound <= counter <= self.upper_bound and len(current_place.bees) > 0:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn. But it has huge health value"""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eats the whole bee he can reach and sleep for 3 turns. 
    After 3 turns, if there are no bees he can reach, he will do nothing instead.
    """
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0:
            if len(self.place.bees) == 0:
                ...
                remove_bee = random_bee(self.place.bees)
                remove_bee.reduce_health(remove_bee.health)
...
class TankAnt(ContainerAnt):
    """TankAnt not only can protect the contained ant, but also attack all the bees in current place"""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower can be placed in the water"""
    name = "Scuba"
    implemented = True
    is_waterproof = True
    food_cost = 6
    ...
    def action(self, gamestate):
        ...
        while current_place != None:
            if current_place.ant != None:
                ...
                elif current_place.ant.is_container:
                    current_place.ant.double()
                    current_place.ant.is_doubled = True
